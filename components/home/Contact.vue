    ...
    <div class="relative mx-auto max-w-7xl lg:grid lg:grid-cols-5">
      <div class="bg-gray-100 py-16 px-4 sm:px-6 lg:col-span-2 lg:px-8 lg:py-24 xl:pr-12">
        <div class="mx-auto max-w-lg h-full flex flex-col">
          ...
          <p class="mt-3 text-lg h-full leading-6 text-gray-500">Entre em contato através do email, WhatsApp ou me ligue e mande
            suas dúvidas, solicite um orçamento. Responderei o mais breve possível.</p>
        ...
        <div class="mx-auto max-w-lg lg:max-w-none">
          <form action="#" method="POST" class="grid grid-cols-1 gap-y-6" @submit.prevent="">            
            <div v-for="input in formInputs" :key="input.name">
              ...
              <input v-if="input.type === 'text'" :id="input.name" :name="input.name" :type="input.type" :autocomplete="input.autocomplete"
            ...
            <div>
              <button type="submit"
                class="inline-flex justify-center rounded-md border border-transparent bg-indigo-600 py-3 px-6 text-base font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Submit</button>
            </div>
          </form>
        </div>
      </div>
